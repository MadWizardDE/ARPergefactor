...
Also your endeavors must be limited by the electricity bill or your awareness of climate change. Having only the devices powered on that are actual needed is not only cost efficient, but also smart.
...
- Support for virtual machines
    ...
    - Traditional WakeOnLAN packets will be redirected to the physical host
...
But if you just have two devices, for example a laptop and a server to which want to connect, you can also run ARPergefactor right there on you client and configure it to only react to connection attempty made by the very same device.

This also guarantees, that if you dwell in a bigger network topology, you can still wake your favourite devices on demand, but don't interfere with the other hosts on the that network.
...
### Static IP configuration or Autoconfig
